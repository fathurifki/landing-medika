...
const FILES_BASE = (process.env.NEXT_PUBLIC_IMAGE_URL || "http://localhost:3001/files").replace(/\/files$/, "");
...
export const filesApi = axios.create({ baseURL: FILES_BASE });
...
filesApi.interceptors.request.use(attachToken);
